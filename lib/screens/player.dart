...
class _PlayerState extends State<Player> with RouteAware, WidgetsBindingObserver {
          ...
          Flexible(
            child: AppTooltip(
              ...
              child: InkWell(
                ...
                child: Text(_song.toString(), style: appBarTextStyle),
              ),
            ),
          ),
        ],
        ...
        Flexible(
          child: AppTooltip(
            ...
            child: InkWell(
              ...
              hoverColor: hoverColor,
              child: Text(' by  ${_song.artist}', style: appBarTextStyle, softWrap: false),
            ),
          ),
        ),
                        ...
                        AppWrap(
                          children: [
                            ...
                            AppTooltip(
                              ...
                              child: appIconWithLabelButton(
                                icon: appIcon(Icons.settings, size: 1.5 * _fontSize),
                                onPressed: () {
                                  _settingsPopup();
                                },
                              ),
                        ...
                        if (app.isScreenBig && !_songUpdateService.isFollowing)
                          ...
                          AppWrap(
                            children: [
                              AppTooltip(
                                message:
                                    'Beats per minute.  Mouse click here or tap the m key\n'
                        ...
                        if (app.isScreenBig && _songUpdateService.isFollowing)
                          AppTooltip(
                            message:
                                ...
                                'To correct this from the main screen: menu (hamburger), Options, Hosts: None',
                            child: Text('BPM: ${playerSelectedBpm ?? _song.beatsPerMinute}', style: _headerTextStyle),
                          ),
                        AppTooltip(
                          message:
  ...
  _updateCountIn(int countIn) {
    ...
    if (countIn > 0 && countIn < countInMax) {
      _countInWidget = Container(
        ...
        child: Text(
          'Count in: $countIn',
          style: _lyricsTable.lyricsTextStyle.copyWith(
  ...
  _bpmBump(final int bump) {
    ...
    _changeBPM(
      (playerSelectedBpm ?? _song.beatsPerMinute) + bump.sign * _bumpSteps[Util.indexLimit(_bpmBumpStep, _bumpSteps)],
    );
      ...
      case SongUpdateState.drumTempo:
        //  followers get to follow even if not in play
        {
          ...
          if ((minItemPosition?.index ?? 0) == 1 && minItemPosition?.itemLeadingEdge == 0.0) {
            ...
            logger.log(_logScrollListener, 'reposition to top: ${minItemPosition?.index}');
          } else if ((maxItemPositionIndex ?? -1) >=
              _lyricsTable.songMomentNumberToGridRow(_song.songMoments.length - 1)) {
            //  scroll to the bottom of the list
            _lastPlayMomentNumber = _song.songMoments.length - 1;
  ...
  _selectMoment(final int momentNumber) {
    ...
    logger.log(
      ...
      'manualPlay sectionRequest: index: $_lyricSectionNotifier'
      ', row: $row, momentNumber: $momentNumber',
    );
  ...
  _selectLyricSection(int lyricSectionIndex) {
    ...
    logger.log(
      _logManualPlayScrollAnimation,
      '_selectLyricSection: lyricSectionIndex: $lyricSectionIndex'
      ', row: ${_lyricsTable.lyricSectionIndexToRow(lyricSectionIndex)}',
    );
  ...
  void _setPlayState() {
    if (_songUpdate != null && _song.songMoments.isNotEmpty && !_songUpdateService.isLeader) {
      ...
      switch (update.state) {
        ...
        default:
          ...
          _itemScrollToRow(_lyricsTable.songMomentNumberToGridRow(songMoment.momentNumber));
          break;
      }
